...
#4. Train
def train(model, processor, dataloader,valid_loader,test_loader, optimizer, schedular, device, epoch,num_epoch,extracted_string,best_acc,best_submission):
    model.train()
    ...
    with tqdm(dataloader, total=len(dataloader), desc=f'Epoch {epoch + 1}') as pbar:
        for i, (images, questions, answers, mode_answer) in enumerate(pbar):
            ...
            if (i+1)%5000==0:
                ...
                if best_acc < valid_acc and valid_acc>0.74:
                    best_acc = valid_acc
                    # 提出用ファイルの作成
                    model.eval()
                    submission = []
                    ...
                    np.save(f"submissions/submission_PaliGemmaFT_{extracted_string}_epoch_{epoch + 1}_{i+1}.npy", submission)
                    print(f"successfully saved model and submission\n")
                    best_submission = submission
                model.train()

    return total_loss / len(dataloader), time.time() - start, best_acc, best_submission
#5. Valid
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=1, shuffle=True, collate_fn=collate_fn, drop_last=True, num_workers=4)
    valid_loader = torch.utils.data.DataLoader(valid_dataset, batch_size=1, shuffle=True, collate_fn=collate_fn, drop_last=True, num_workers=4)
    ...
    model_name="google/paligemma-3b-ft-vqav2-448" # init_acc = 0.0921(0.3473), 0.5912, 0.7121, 0.7296
    # 0.3618,0.7348,0.7331,0.7441,0.7343
    #       ,0.7439,0.7406,0.7356

    # loss
    # lora 0.6470,0.4137,0.2415,0.1243
    # lora+ 0.6380,0.3221,0.1523
    # https://github.com/nikhil-ghosh-berkeley/loraplus

    match = re.search(r'ft-(.*?)-448', model_name)
    if match:
        extracted_string = match.group(1) +"-448-loraplus"
    ...
    for epoch in range(num_epoch):
        train_loss, train_time, best_acc, best_submission = train(model, processor, train_loader, valid_loader, test_loader, optimizer, scheduler, device, epoch,num_epoch,extracted_string, best_acc, best_submission)
        print(f"【{epoch + 1}/{num_epoch}】\n"
